...
const decrementCredtByEmailUser = async (
  email: string,
): Promise<void> => {
  try {
    ...
    const updatePromises = userByEmail.docs.map(async (doc) => {
      try {
        await firestoredataBase
          ...
          .doc(doc.id)
          .update({
            creditQuestion: FieldValue.increment(-1), // Decrementa em uma unidade os créditos
          });
        console.log(
...
export default decrementCredtByEmailUser;
